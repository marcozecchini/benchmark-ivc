        ...
        From<<FS as FoldingScheme<C1, C2, FC>>::VerifierParam>,
{
    ...
    fn preprocess(
        ...
        let circuit = DeciderEthCircuit::<C1, C2>::dummy((
            ...
            // LOCAL PATCH (TransLog): the decider circuit always verifies a
            // 1x1 NIMFS fold (U_i + u_i, see fold_field_elements_gadget), so the
            // dummy proof must be shaped (1, 1). Upstream uses (MU, NU), which
            // panics at Groth16 keygen for MU > 1 (index out of bounds in
            // compute_c_gadget); upstream only ever tests MU = NU = 1.
            (s, t, 1, 1),
            (),
            state_len,
