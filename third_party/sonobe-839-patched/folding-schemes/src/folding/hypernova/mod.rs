    ...
    C1: Curve<BaseField = C2::ScalarField, ScalarField = C2::BaseField>,
{
    ...
    fn prove_step(
        ...
        if self.i == C1::ScalarField::zero() {
            ...
            if std::env::var("HN_PROFILE").is_ok() {
                eprintln!("[hn-profile] nimfs_prove (native multifold + sumcheck): {:?}", _prof_t.elapsed());
            }
            ...
            self.cf_U_i = cf_U_i1;
            if std::env::var("HN_PROFILE").is_ok() {
                eprintln!("[hn-profile] cyclefold (cf synthesis + Pedersen commit + NIFS): {:?}", _prof_t.elapsed());
            }
        }
        ...
        let cs = cs.into_inner().ok_or(Error::NoInnerConstraintSystem)?;
        if std::env::var("HN_PROFILE").is_ok() {
            eprintln!("[hn-profile] augmented circuit witness synthesis: {:?}", _prof_t.elapsed());
        }
        ...
        if std::env::var("HN_PROFILE").is_ok() {
            eprintln!("[hn-profile] to_cccs (witness commit, KZG MSM): {:?}", _prof_t.elapsed());
        }
        self.u_i = u_i.clone();
        self.w_i = w_i.clone();
